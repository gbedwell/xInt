...
#'
#' @return A list of data frames containing the sequence position and the corresponding dG value for each element in sites.
#'
#' @examples
#' library(BSgenome.Hsapiens.UCSC.hs1)
#' data(sites2)
#' calculate_dG(sites = sites2, genome.obj = Hsapiens)
#'
#' @import ggplot2
...
#' @export
#'
calculate_dG <- function(sites, step.len = 50, genome.obj,
  ...
  if(isTRUE(collapse)){
    dG <- Map(f = function(x, y) { cbind(x, dataset = y) }, dG, names(dG))
    dG <- do.call(rbind, dG)
    rownames(dG) <- NULL
  }

...
subtract_mask <- function( reference, mask ){
  ...
  diffranges <- GenomicRanges::subtract( reference, mask, minoverlap = 1L, ignore.strand = TRUE )
  diffranges <- unlist( diffranges )
  overlaps <- findOverlaps( query = reference, subject = diffranges, minoverlap = 1L, ignore.strand = TRUE )

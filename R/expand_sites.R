#' Expand Site Coordinates
#'
#' Expands site coordinates by a defined amount.
#'
#'@param sites The list or GRangesList containing the mapped site coordinates.
...
#'
expand_sites <- function(sites, seq.len = 50, genome.obj, current.start = 1, tsd = 5){
  ...
  ex <- lapply(
    ...
    FUN = function(x){
      ...
      if(length(outs) != 0){
        warning(
          "Expanded coordinates " , paste(outs, collapse=", "),

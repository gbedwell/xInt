...
make_fragments <- function(insert.sites,
  ...
  if(!isTRUE(random)){
    ...
    matches <- lapply(
      ...
      FUN = function(x){
        if(isTRUE(U3)){
          ...
          strand(tmp.sites) <- ifelse(strand(tmp.sites) == '+', '-', '+')
          }

          precede(
            x = ifelse(isTRUE(U3), tmp.sites, insert.sites),
            subject = x,
            select = "all",
            ignore.strand = FALSE
            )
      ...
      FUN = function(x){
        ...
        if(!isTRUE(U3)){
          plus <- GRanges(
           ...
           minus <- GRanges(
             seqnames = seqnames(is[minus.ind]),
    ...
    if(isTRUE(to.chr.ends)){
      # Add a check for pos.seqnames and neg.seqnames being length > 0
    ...
    plus.widths <- round(rlnorm(n = length(plus.sites), meanlog = lnorm.loc, sdlog = lnorm.shape))

    if(!isTRUE(U3)){
      plus.sites <- GRanges(
        seqnames = seqnames(plus.sites),
    ...
    minus.widths <- round(rlnorm(n = length(minus.sites), meanlog = lnorm.loc, sdlog = lnorm.shape))
    ...
    outliers <- bound_check(
      fragments = frag.ranges,
  ...
  frag.ranges$name <- paste0(read.name.prefix, seq_along(frag.ranges))
